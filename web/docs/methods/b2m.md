...
用来检验解对网格的依赖性 —— 这正是 `meshtest/` 里 E7 那组实验的原型。
...
实测（`meshtest/` 的 E1，对面梯度型泛函与解析积分比较）：
...
验法只用一条判据（E7，$\lambda=0$ 完全没有正则化）：真值取整块均匀磁化、磁场无噪声，
...
!!! warning "尚未接入生产"
    `meshtest/b2m_core.py` 里已经有 `variable="m" | "sqrt_v_m"` 开关（`VARIABLES`、
    `variable_scale()`、`to_m()`），但**生产 notebook 的未知量仍然是 $m_i$**。
...
| 点偶极近似 | 测点离磁体表面太近（< 4~6 个单元尺度）时误差不可忽略；`meshtest` 里踩过"真值处残差 = 90σ"的坑 |
